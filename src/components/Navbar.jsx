    ...
    <nav className='fixed top-0 left-0 w-full z-50'>
      ...
      md:rounded-xl backdrop-blur-lg'>
        <div className='text-white font-semibold text-lg uppercase'>
            <a href="/">soumodip ghosh</a>
        </div>
        <div className='hidden md:flex space-x-8'>
            {LINKS.map((link, index) => (
                <a href={link.href} key={index} className="text-white hover:text-stone-400 transition
                duration-300">{link.label}</a>
            ))}
        </div>
      ...
      {menuOpen && (
        <div className='md:hidden p-2 bg-stone-950/30 backdrop-blur-lg rounded-xl flex flex-col space-y-4
        max-w-6xl mx-auto'>
          {LINKS.map((link, index) => (
